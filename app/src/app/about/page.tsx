...
const RESPONSIBILITY_ROWS = [
  {
    area: "商品・販売店情報",
    apiRole: "物件情報、販売店、ファイナンス可否、所有権/担保確認ステータスを返却する",
    outsideRole: "商品マスタ登録、写真・整備履歴の正確性担保、販売店審査はEC事業者/リース会社が実施",
  },
  {
    area: "月額試算",
    apiRole: "サーバー側の商品価格・料率テーブルを使い、月額、残価、保険/メンテ費用、見積IDを返却する",
    outsideRole: "最終条件、キャンペーン料率、個別稟議条件はリース会社の審査/承認プロセスで確定する",
  },
  {
    area: "仮審査申込",
    apiRole: "法人情報、同意、見積IDを受け付け、リスクスコア、仮承認/追加確認/否決、必要書類を返却する",
    outsideRole: "本人確認、反社チェック、信用情報照会などの外部照会結果はAPI内部連携または社内運用で補完する",
  },
  {
    area: "本審査・ステータス管理",
    apiRole: "許可されたステータス遷移だけを受け付け、進捗、次アクション、タイムラインをEC側へ返却する",
    outsideRole: "書類確認、現地確認、稟議、決裁、条件交渉はリース会社の担当者/社内ワークフローで実施する",
  },
  {
    area: "検収・販売店支払",
    apiRole: "検収完了を記録し、検収済み案件だけ販売店支払予定を作成して返却する",
    outsideRole: "実際の納品確認、支払承認、銀行振込、支払取消/組戻しはリース会社の支払システムで実施する",
  },
  {
    area: "請求・債権管理",
    apiRole: "請求開始ステータス、契約満了までの進捗をEC側に連携できる状態で保持する",
    outsideRole: "月額請求、口座振替、入金消込、延滞督促、回収業務は基幹/債権管理システムが担当する",
  },
];

const API_BOUNDARY_POINTS = [
  { label: "APIが担う", detail: "EC画面から呼び出せる試算、申込、進捗照会、検収、支払予定作成のインターフェース" },
  { label: "APIが制御する", detail: "価格改ざん防止、見積と申込の紐付け、ステータス遷移、検収後支払などの業務ルール" },
  { label: "APIが返す", detail: "月額、料率内訳、審査結果、必要書類、次アクション、タイムライン、支払予定" },
  { label: "API対象外", detail: "最終与信判断、契約締結、銀行振込、請求/回収、監査保管などの社内・基幹業務" },
];
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8 space-y-16">
      ...
      <section className="space-y-6">
        ...
        <p className="text-sm text-text-secondary">
          エンベデットリースAPIは、EC事業者が組み込む公開インターフェースとして、月額試算、申込受付、審査進捗、検収、販売店支払予定までを一貫して扱います。
          一方で、最終与信、契約、銀行振込、請求回収などの実務処理は、リース会社の社内システムや担当者業務と連携する前提です。
        </p>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
          {API_BOUNDARY_POINTS.map((point) => (
            <div key={point.label} className="bg-white rounded-lg border border-border p-4">
              <p className="font-bold text-sm text-primary">{point.label}</p>
              <p className="text-xs text-text-secondary mt-2 leading-relaxed">{point.detail}</p>
            </div>
          ))}
        </div>
        ...
        <div className="bg-white rounded-lg border border-border overflow-hidden">
          <div className="px-4 py-3 bg-slate-50 border-b border-border">
            <h3 className="font-bold">担当範囲の整理</h3>
            <p className="text-xs text-text-secondary mt-1">APIはEC接点の標準化と業務ルールの制御を担い、社内判断や資金移動はリース会社側の運用に接続します。</p>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-50 border-b border-border">
                  <th className="px-4 py-3 text-left font-bold w-40">領域</th>
                  <th className="px-4 py-3 text-left font-bold">APIが担当すること</th>
                  <th className="px-4 py-3 text-left font-bold">API外/連携先が担当すること</th>
                </tr>
              </thead>
              <tbody>
                {RESPONSIBILITY_ROWS.map((row) => (
                  <tr key={row.area} className="border-b border-border last:border-b-0 align-top">
                    <td className="px-4 py-3 font-bold text-primary bg-slate-50/60">{row.area}</td>
                    <td className="px-4 py-3 text-text-secondary leading-relaxed">{row.apiRole}</td>
                    <td className="px-4 py-3 text-text-secondary leading-relaxed">{row.outsideRole}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <p className="text-sm text-text-secondary">
          下図はシステム構成上の見方です。EC事業者は公開APIだけを呼び出し、外部照会や社内業務はAPI内部またはリース会社側のシステムに隠蔽されます。
        </p>
